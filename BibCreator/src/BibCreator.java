...
public class BibCreator {
	...
	//we create an arrayList to store articles of each file
    private static ArrayList<ArrayList<Article>> articlesByFiles;
    
    //sort invalid file number using array list
    private static ArrayList<Integer> invalidFileNum;
    
    //A declaration of 10 files to read
    ...
    public static void main (String []args){
        ...
        for(int i=1; i<=N; i++) {
            try {
                ...
                System.out.println("Could not open the input file "+file+i+".bib for reading. \n" +
        ...
        //checking the input file for validation
        for(int i=1; i<=N; i++){
            Article article = null;
            BufferedReader bf = null;
            try {
                bf = new BufferedReader(new FileReader(file+i+".bib"));
                boolean result = processFilesForArticles(bf, article, i);
                if(!result){
                    invalidFileNum.add(i);
                }
            } catch (FileNotFoundException e) {
            	//prints the line number and class name where the exception occurred
                e.printStackTrace();
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private static void writeinNJformat(PrintWriter pw, int j) {
        if(!invalidFileNum.contains(j)) {
            for (int i = 0; i < articlesByFiles.get(j).size(); i++) {
                ...
                //the replace() method replaces the and for &
    ...
    private static void writeinACMformat(PrintWriter pw, int j) {
        if(!invalidFileNum.contains(j)) {
            for (int i = 0; i < articlesByFiles.get(j).size(); i++) {
                ...
                // the split() method divides the string and returns an array of substrings
